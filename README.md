...
面向大学计算机课程的 AI 实验工作台。学生上传课件、实验数据、代码和笔记后，通过智能任务路由与受控的 Agent 模式获得基于真实资料的 AI 回答，并把有价值的回答保存为可导出的成果。
...
LumenLab 是一个面向大学生与学习者的 AI 学习平台，围绕"项目"组织课程资料。

- 通过智能任务路由自动识别用户意图，覆盖实验报告、代码调试、课件总结等 16 种任务类型。
- 通过受控的 Agent 模式让模型以 Skill 绑定 + Tool 调用的方式完成多步任务，所有副作用由服务端 Policy Engine 审批。
- 把有长期价值的回答保存为 Markdown、DOCX 或 PDF 成果。
...
上传文件或输入问题后，系统通过关键词规则引擎自动判定 16 种任务类型，不需要手动切换模式。零 token 消耗，响应即时。
...
- L2 会话预批准：`artifact.save`。
- L3 每次询问：`project_files.delete`。
- Skill 只能收紧权限，不能放宽。
...
| Tool | 分类 | 风险 |
|------|------|------|
| project_files.list | project-files | L1 |
| project_files.read | project-files | L1 |
| project_files.delete | project-files | L3 |
| artifact.save | artifacts | L2 |
...
- 支持把转换结果保存为项目资料，图片复制为项目独立资源。

### 降级 RAG 检索

三级检索策略按优先级：用户选中文件、关键词全文搜索、pgvector 向量检索。可以在没有外部 embedding 服务的情况下正常工作，向量检索接入后自动提升精度。
...
| 文件解析 | PDF.js, MiniMax M3 OCR, @napi-rs/canvas |
...
│   ├── skills/                         # 内置 Skill 包
│   │   ├── paper-writer/manifest.ts
│   │   ├── exam-coach/{manifest,instructions}.ts
│   │   ├── code-reader/manifest.ts
│   │   ├── registry.ts                 # 注册 + Provider-aware 工具 payload
│   │   └── executor.ts                 # 旧 SkillDefinition 兼容层
│   ├── tools/                          # 内置 Tool 实现
