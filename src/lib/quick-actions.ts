...
export const DEFAULT_QUICK_ACTIONS: Record<ProjectType, QuickActionDefinition[]> = {
  ...
  review: [
    ...
    { title: "生成 Mermaid 思维导图", prompt: "请基于我选中的资料生成 Mermaid flowchart 逻辑图，使用 flowchart LR（从左到右深度顺序）或 flowchart TD（从上到下先后顺序）。要求：1）每个节点 classDef 使用 whitebox 样式（白底+彩色边框+黑字）；2）按逻辑依赖关系排列，同级并列，子级递进；3）节点标签中不要使用半角括号 () 逗号 , 竖线 |，改用全角符号或方括号；同时给出可直接复制的 Mermaid 代码块，不要添加资料外的知识点。" },
  ],
  coding: [
