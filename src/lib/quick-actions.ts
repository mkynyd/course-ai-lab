...
export const DEFAULT_QUICK_ACTIONS: Record<ProjectType, QuickActionDefinition[]> = {
  ...
  review: [
    ...
    { title: "生成 Mermaid 逻辑图", prompt: "请基于我选中的资料生成 Mermaid flowchart TD（从上到下纵向排列），按逻辑依赖排列层级，同级并列，子级递进。严格遵守：1）节点标签只使用纯中文或英文文字，绝对禁止在标签内使用半角括号 ()、大括号 {}、尖括号 <>、脱字符 ^、竖线 |，这些符号会直接导致渲染崩溃；2）所有标签用方括号 [标签] 或圆角方括号 (标签) 包裹；3）每个节点语句独占一行。同时给出可直接复制的 Mermaid 代码块。" },
  ],
  coding: [
