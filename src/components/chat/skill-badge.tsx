...
export function SkillBadge({ skill, className }: SkillBadgeProps) {
  return (
    <div
      className={cn(
        "inline-flex items-center gap-1.5 rounded-full bg-[var(--color-accent)]/12 px-2.5 py-1 text-xs font-medium text-[var(--color-accent)]",
        className
      )}
    >
      <Sparkles size={10} />
      <span>{skill.skillId}</span>
      <span className="font-mono text-xs opacity-70">{skill.version}</span>
    </div>
  );
}
